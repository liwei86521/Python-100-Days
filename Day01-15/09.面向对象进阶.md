...
'''
第二种方法中，使用了@property装饰器将getter方法变成了属性
此时@property又创建了另一个装饰器@score.setter，负责把setter方法变成属性赋值。
这样，就把getter和setter方法装饰成了属性，
在调用getter和setter方法的时候就像是在调用该类的属性那样，更直接。
'''
...
class Person(object):
    ...
    __slots__ = ('_name', '_age', '_gender') # 可以减少很多内存占用
...
class Clock(object):
    ...
    def show(self):
        ...
        # 推荐用 f-string     
        return f'{self._hour} 时   {self._minute} 分  {self._second} 秒' 
