...
1. 数据结构和算法
   ...
   - 排序算法（选择、冒泡和归并）和查找算法（顺序和折半）
     ...
     ```Python
     def bubble_sort_v1(origin_items):
       ''' 最基本的 冒泡算法'''
        items = origin_items[:]
     ...
     print(bubble_sort_v1(origin_items=[3,4,6,1,9,8]))
...
5. 并发编程
   ...
   - 多线程：Python中提供了Thread类并辅以Lock、Condition、Event、Semaphore和Barrier。Python中有GIL来防止多个线程同时执行本地字节码，这个锁对于CPython是必须的，因为CPython的内存管理并不是线程安全的，因为GIL的存在多线程并不能发挥CPU的多核特性。
     ...
     p = Path(r"C:\Users\F7687778\Desktop\test.jpeg")

     def generate_thumbnail_v2(infile=p, size=(200, 150), format='PNG'):
         """生成指定图片文件的缩略图"""
         file, ext = infile.stem, infile.suffix
         print(file, ext) # test .jpeg

         outfile = f'{file}_{size[0]}_{size[1]}{ext}'
         img = Image.open(infile)
         img.thumbnail(size, Image.ANTIALIAS)
         img.save(outfile, format)
